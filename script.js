(function () {
  ...
  // ====== CONSTANTS ======
  const LIB_KEY   = 'Nexapedia.library.v1';
  const THEME_KEY = 'Nexapedia.theme.v1';
  ...
  const DICT_ID_BASE  = null; // disabled
// 'https://kbbi-api-zhirrr.vercel.app/api/kbbi?text=';
  ...
  const translations = {
    id: {
      ...
      infoText:
        'Nexapedia mengambil ringkasan langsung dari <b>Wikipedia Indonesia &amp; Inggris</b>, makna kata Inggris dari <b>Free Dictionary API</b>, dan makna kata Bahasa Indonesia dari <b>New KBBI API</b>. Gunakan mode <b>Ensiklopedia</b> untuk konsep umum dan <b>Kamus</b> untuk arti kata.',
      ...
      articleIntroWiki:
        'Nexapedia akan menampilkan ringkasan ringkas dari Wikipedia tentang topik yang kamu cari. Gulir ke bawah untuk melihat tautan penelitian dan referensi lanjutan.',
      articleIntroDict:
    ...
    en: {
      ...
      recBadge: 'From wiki',
      infoText:
        'Nexapedia fetches summaries from <b>Indonesian &amp; English Wikipedia</b>, English meanings from <b>Free Dictionary API</b>, and Indonesian meanings from <b>New KBBI API</b>. Use <b>Encyclopedia</b> mode for concepts and <b>Dictionary</b> mode for word meanings.',
      ...
      articleIntroWiki:
        'Nexapedia shows concise summaries from Wikipedia about the topic you search for. Scroll down for research links and extended reading suggestions.',
      articleIntroDict:
  ...
  // Terjemahkan teks Indonesia ke Inggris untuk pencarian wiki
  async function translateIdToEn(text) {
    try {
      ...
      console.error('Gagal menerjemahkan teks ke Inggris:', e);
    }
    return text;
  }
  ...
  document.getElementById("downloadPdfBtn").addEventListener("click", () => {
    ...
    const url = `https://${lang}.wikipedia.org/api/rest_v1/page/pdf/${encodeURIComponent(title)}`;
  ...
  async function loadResearchPapers(query) {
    ...
    try {
      const url = `https://api.semanticscholar.org/graph/v1/paper/search?query=${encodeURIComponent(query)}&limit=5&fields=title,year,authors,url`;

      const resp = await fetch(url);
      if (!resp.ok) throw new Error("Semantic Scholar API error");
      ...
      data.data.forEach(paper => {
        ...
        const year = paper.year || "Tahun tidak diketahui";
        ...
        item.innerHTML = `
          <div class="rr-item-title">${paper.title}</div>
          <div class="rr-item-meta">${authors} • ${year}</div>
        `;
      ...
      console.error("Gagal mengambil riset:", err);
    }
  }
  ...
  async function loadArticle(title, opts) {
    ...
    try {
      ...
      // riset lanjutan
      loadResearchPapers(effectiveTitle);
  ...
  // ====== DICTIONARY LOADER (ENGLISH ONLY) ======
  function loadDictionary(word, opts) {
    if (!word) return;
    const options = Object.assign({ showLoading: true }, opts || {});

    articleContent.classList.remove('show');
    if (options.showLoading) setLoading(true, 'dict', word);
    ...
    if (!isEnglish) {
      ...
      articleExtract.innerHTML = `
        <p style="opacity:.85;font-size:13px;">
          Mode kamus hanya mendukung <b>kata bahasa Inggris</b>.<br>
          Silakan gunakan mode Ensiklopedia untuk kata atau topik lain.
        </p>
      `;
      clearArticleExtras();
      articleContent.classList.add('show');
    ...
    const url = DICT_EN_BASE + encodeURIComponent(word);
    ...
    fetch(url)
      ...
      .then(data => {
        const trans = t();
        articleTitle.textContent = data[0]?.word || word;
        articleMeta.innerHTML = '<span>' + trans.articleMetaDict + '</span>';
        renderDictionaryResultEn(word, data);
        clearArticleExtras();
        attachResearchLinks(word);
        sourceValue.textContent = trans.dictLangLabelEn;
        setLoading(false);
        requestAnimationFrame(() => articleContent.classList.add('show'));
      })
      .catch(err => {
        ...
        articleExtract.innerHTML = trans.loadErrorBodyDictEn(word);
        clearArticleExtras();
        attachResearchLinks(word);
        articleContent.classList.add('show');
      });
  ...
  async function triggerSearch() {
    ...
    clearActiveChips();
    if (currentMode === 'wiki') {
      // Selalu gunakan kata kunci bahasa Inggris untuk pencarian wiki.
      // Jika bahasa UI Indonesia, terjemahkan terlebih dahulu.
      const query = currentLang === 'id' ? await translateIdToEn(val) : val;
      loadArticle(query, { from: 'search' });
    } else {
      loadDictionary(val, { from: 'search' });
    }
  }
  ...
  if (bgMusic) {
    // AUTOPLAY SETELAH USER INTERACTION PERTAMA
